...
namespace Anim
{
  ...
  static Transformation transformationInterpolation(const Transformation & t1,
						    ...
						    float weight)
  {
    ...
    Transformation t;
    t.use_spline = false;
    t.s = t1.s + weight * (t2.s - t1.s);
    t.v.x = t1.v.x + weight * (t2.v.x - t1.v.x);
    t.v.y = t1.v.y + weight * (t2.v.y - t1.v.y);
    t.v.z = t1.v.z + weight * (t2.v.z - t1.v.z);
    return t;
  }
