...
{
  //
  // FIXME : rewrite branchless
  //
 //  const unsigned int zero = !(!b);
//   const unsigned int casted_a = *(unsigned int *)&a;
//   const unsigned int sign_mask = ((casted_a >> 31 - 1)) | ((*(unsigned int *)&b) << 24);
//   const unsigned int casted_result = casted_a ^ sign_mask;
//   return (*(float *)&casted_result);
  if (b > 0)
    return a;
  if (b < 0)
    return -a;
...
float			Noise(float x, float y, float z, float t)
{
  ...
  dx1 = UnsecureSpline5_interpolation(dx1);
  dy1 = UnsecureSpline5_interpolation(dy1);
  dz1 = UnsecureSpline5_interpolation(dz1);
  dt1 = UnsecureSpline5_interpolation(dt1);

  const float b111 = UnsecureLinear_interpolation(b1110, b1111, dt1);
  const float b110 = UnsecureLinear_interpolation(b1100, b1101, dt1);
  const float b101 = UnsecureLinear_interpolation(b1010, b1011, dt1);
  const float b100 = UnsecureLinear_interpolation(b1000, b1001, dt1);
  const float b011 = UnsecureLinear_interpolation(b0110, b0111, dt1);
  const float b010 = UnsecureLinear_interpolation(b0100, b0101, dt1);
  const float b001 = UnsecureLinear_interpolation(b0010, b0011, dt1);
  const float b000 = UnsecureLinear_interpolation(b0000, b0001, dt1);

  const float b11 = UnsecureLinear_interpolation(b110, b111, dz1);
  const float b10 = UnsecureLinear_interpolation(b100, b101, dz1);
  const float b01 = UnsecureLinear_interpolation(b010, b011, dz1);
  const float b00 = UnsecureLinear_interpolation(b000, b001, dz1);

  const float b1 = UnsecureLinear_interpolation(b10, b11, dy1);
  const float b0 = UnsecureLinear_interpolation(b00, b01, dy1);

  return UnsecureLinear_interpolation(b0, b1, dx1);
}
