...
void load_lsystem(char *file)
{
    ...
    const date storyNow = intro.now.story;
    glPushMatrix();
    l = new LSystem::System(*intro.sceneRenderList, storyNow + 1000, storyNow,
			    Shader::parallax/*default*/, Anim::none, Texture::none,
...
{
  const Transformation & t1 = intro.cameraAnim->getPrevTransformation(intro.now.camera, 0);
  const Transformation & t2 = intro.cameraAnim->getNextTransformation(intro.now.camera, 0);
...
//     break;
  ...
  case KEY_G:					// Caméra/lumière auto -> manuelle
    ...
    else
    {
      ...
      date animDate = intro.now.camera;
...
void intro_mouse(const input_mouse & mouse, bool shift, bool ctrl)
{
  ...
  translateCamera(0, /* (shift ? 0.1f : 1.f) */ 10.f * mouse.dz, 0);
}
