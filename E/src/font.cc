...
float
Font::width(unsigned char c, float height) const
{
...
float
Font::height(unsigned char c, float width) const
{
  const float ratio = width / width_[c - 32].abcfB;

  return size_y_ * ratio;
}

float
Font::width(const char* str, float height) const
{
...
float
Font::height(const char* str, float width) const
{
  const float ratio = this->width(str, (float)size_y_);

  assert(ratio != 0);

  return (width * size_y_) / ratio;
}


void
Font::print(const char* str, float height, float x, float y) const
{
  if (!str || !*str)
    return;
...
void
Font::print_w(const char* str, float width, float x, float y) const
{
  if (!str || !*str)
    return;

  const unsigned char* text = (unsigned char*) str;
  float ratio = height(str, width) / (float)size_y_;

  _print(str, ratio, x, y);
}

void
Font::_print(const char* text, float ratio, float x, float y) const
{
  glMatrixMode(GL_MODELVIEW);
  glPushMatrix();
