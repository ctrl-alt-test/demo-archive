...
uniform vec3 emissiveMultiplier;
...
void main()
{
  ...
  vec4 emissiveRGBD = texture2D(emissiveTexture, uv);
  vec3 emissive = emissiveMultiplier * emissiveRGBD.rgb / emissiveRGBD.a;

  vec3 color = emissive + albedo_A.rgb * ambientLight +
    mix(albedo_A.rgb * ambientLight, ambientLight, Schlick(f0, E, N));
