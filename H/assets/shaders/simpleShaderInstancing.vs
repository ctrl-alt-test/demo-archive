...
void main()
{
  vec3 position = vec3(2.0 * float(gl_InstanceID % gridSize) - float(gridSize),
		       2.0 * float((gl_InstanceID / gridSize) % gridSize) - float(gridSize),
		       2.0 * float(gl_InstanceID / (gridSize * gridSize)) - float(gridSize));

  mat4 rotationMatrix = getRotationMatrix(sin(float(gl_InstanceID)) * time, normalize(position));
  vec4 movedVertex = rotationMatrix * (vec4(size, size, size, 1.0) * vec4(vertexPosition, 1.0));
  movedVertex.xyz += position;

  instanceId = gl_InstanceID;
  texCoord = vertexTexCoord;
  normal = normalize(mat3(modelMatrix) * (mat3(rotationMatrix) * vertexNormal));
  vec4 worldSpacePosition = modelMatrix * movedVertex;
  ...
  shadowCoord[0] = modelLightProjectionMatrix[0] * movedVertex;
  shadowCoord[1] = modelLightProjectionMatrix[1] * movedVertex;
  shadowCoord[2] = modelLightProjectionMatrix[2] * movedVertex;
  shadowCoord[3] = modelLightProjectionMatrix[3] * movedVertex;
  shadowCoord[4] = modelLightProjectionMatrix[4] * movedVertex;
