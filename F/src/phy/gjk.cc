...
namespace phy
{
  ...
  namespace
  {
    ...
    inline vector2f project(const vector3f& v0, const vector3f v1, const point3f p)
    {
      ...
      const float v0v0 = v0 * v0;
      const float v0v1 = v0 * v1;
      const float v1v1 = v1 * v1;
      const float v0p  = v0 * p;
      const float v1p  = v1 * p;
    ...
    void data2(gjk_data& d, int np0, const vector3f& v)
    {
      ...
      if (d.vector * d.vector <= epsilon)
      {
        ...
        d.vector = d.vector ^ vector3f(d.vector.y, d.vector.z, d.vector.x);
      }
    }
    ...
    void data3(gjk_data& d, int np0, int np1, const vector3f& v)
    {
      ...
      if (d.vector * d.vector <= epsilon)
        d.vector = (d.sm[1] - d.sm[0]) ^(d.sm[2] - d.sm[0]);
    }
    ...
    void simp2(gjk_data& d)
    {
      ...
      bool f0 = (p01 - p0) * (p1 - p0) < 0.f;
      bool f1 = (p01 - p1) * (p0 - p1) < 0.f;
      ...
      assert(msys_fabsf(v0 * p0) < epsilon);
      assert(msys_fabsf(v1 * p1) < epsilon);
      ...
      {
        assert(msys_fabsf(v0 * p01) < epsilon);
        assert(msys_fabsf(v1 * p01) < epsilon);
        data3(d, 0, 1, p01);
      }
    }
    ...
    bool simp3(gjk_data& d)
    {
      ...
      float k0 = (v0 * (-d.pm)) / (v0 * v0);
      float k1 = (v1 * (-d.pm)) / (v1 * v1);
      float k2 = (v2 * (-d.pm)) / (v2 * v2);
      point3f p0  = d.pm + v0 * k0;
      point3f p1  = d.pm + v1 * k1;
      ...
      bool f01 = (-p01) * (p2 - p01) < 0.f;
      bool f02 = (-p02) * (p1 - p02) < 0.f;
      bool f12 = (-p12) * (p0 - p12) < 0.f;
      ...
      assert(msys_fabsf(v0 * p0) < epsilon);
      assert(msys_fabsf(v1 * p1) < epsilon);
      assert(msys_fabsf(v2 * p2) < epsilon);
      ...
      {
        assert(msys_fabsf(v0 * p02) < epsilon);
        assert(msys_fabsf(v2 * p02) < epsilon);
        data3(d, 0, 2, p02);
      }
      else if (f12)
      {
        assert(msys_fabsf(v1 * p12) < epsilon);
        assert(msys_fabsf(v2 * p12) < epsilon);
        data3(d, 1, 2, p12);
      }
      else
      {
	...
	nd.vector = -nd.sm[3];
      ...
      assert(msys_fabsf((nd.sm[0] + v1 * k1) * v1) < epsilon);
      assert(msys_fabsf((nd.sm[0] + v2 * k2) * v2) < epsilon);
      assert(msys_fabsf(nd.pm * v1) < epsilon);
      assert(msys_fabsf(nd.pm * v2) < epsilon);
    ...
    bool epa_next(Array<gjk_data>& s, gjk_data nd, int index, point3f p1, point3f p2, point3f pm)
    {
      nd.sm[3] = nd.sm[index];
      nd.s1[index] = p1;
      nd.s2[index] = p2;
	...
	epa_init(s, data, 1, 2, 3);
      ...
      for (int i = 0; i < 32 && s.size > 0; ++i) // prevent infinite loops
      {
        ...
        point3f pm = p1 - p2;
	float res = (pm - ref.pm) * ref.vector;
  ...
  bool GJK(Mesh* m1, Mesh* m2, point3f& out1, point3f& out2)
  {
    ...
    for (int i = 0; i < 32; ++i) // prevent infinite loops
    {
      ...
      float res = (data.pm - base) * data.vector;

      DBG("[gjk:%u] support: %f", i, (data.pm * -data.vector));
#ifdef DEBUG
