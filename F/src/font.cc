  ...
  */
  ...
  void Font::_createTexture()
  {
    ...
    for (unsigned int i = 0; i < NB_GLYPHS; ++i)
    {
      ...
      _maxWidth = max(_maxWidth, metrics.gmBlackBoxX);
      _maxHeight = max(_maxHeight, metrics.gmBlackBoxY);
      _maxBaseLine = max(_maxBaseLine, long(metrics.gmBlackBoxY) - metrics.gmptGlyphOrigin.y);
      maxBaseX = max(maxBaseX, metrics.gmptGlyphOrigin.x);
      maxBaseY = max(maxBaseY, metrics.gmptGlyphOrigin.y);
    }
