...
static const char msg_error[] = "Init failed.\n"
...
static void DrawTime( win_info_t *info, float t, long it )
{
    ...
    if( !(frame&3) )
    {
      ...
      sprintf(str, "%s | [%d fps - %d ms/f] - Youtube time %02d'%02d\"%02d (t=%dms) - Story time %02d'%02d\"%02d (t'=%dms)%s",
	      wndclass, fps, it - last_it, m1, s1, c1, intro.now.youtube, m2, s2, c2, intro.now.story, (intro.now.paused? " [PAUSED]" : ""));
      SetWindowText( info->hWnd, str );
    }
    last_it = it;
}
...
//    calcula(); return( 0 );
    ...
    intro_init(XRES, YRES, float(XRES) / float(YRES), true, font, sticker);
