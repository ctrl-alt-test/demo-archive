...
// A deplacer autre part ?
  ...
  // FIXME : inclure je sais pas quoi pour avoir min et max définis
...
#define min(a,b) (((a) < (b)) ? (a) : (b))
  ...
  void		Channel::Dilate(float radius)
  {
	...
	for (int kj = 0; kj < iradius; ++kj)
	  ...
	  {
	    const float factor = 1.f - min(1.f, msys_sqrtf(float(ki * ki + kj * kj)) * invMaxDistance);
	    maxValue = max(maxValue, factor * save[GetIndex(i + ki, j + kj)]);
	    maxValue = max(maxValue, factor * save[GetIndex(i - ki, j + kj)]);
	    maxValue = max(maxValue, factor * save[GetIndex(i + ki, j - kj)]);
	    maxValue = max(maxValue, factor * save[GetIndex(i - ki, j - kj)]);
	  }
