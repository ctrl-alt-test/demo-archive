		...
		while (sc.notenr<bc.notenum && m_state.time==sc.notent)
		{
			...
			*mptr++=(sc.lastvel+=sc.noteptr[4*bc.notenum]);
			...
			UPDATENT2(sc.notenr,sc.notent,sc.noteptr,bc.notenum);
                        ...
                        const double tick2ms = 128. * 1000. / 44100.;
			if (Synchronizing)
				Sync::playNote(sc.lastnte, sc.lastvel, ch, (int)(tick2ms * m_state.time));
		}
		UPDATENT3(sc.notenr,sc.notent,sc.noteptr,bc.notenum);
	}

        // [CAT
#if 0 // support du micro
        static int lastUpdate = 0;
        const double tick2ms = 128. * 1000. / 44100.;
        int time = (int)(tick2ms * m_state.time);
        if (lastUpdate > time || time - lastUpdate > 20) {
          lastUpdate = time;
          for (int ch = 0; ch < 16; ch++)
          {
              char ret[2] = {0, 0};
              Microphone::getVolumePanning(ch, time, ret);
              PUTSTAT(0xb0|ch)
              *mptr++= 4; // volume
              *mptr++= ret[0];
              //PUTSTAT(0xb0|ch)
