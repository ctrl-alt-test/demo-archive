...
int MidiFile::addTrack(void) { 
   ...
   events[length]->setSize(5000);
   events[length]->setSize(0);
   events[length]->allowGrowth(true);
   return length;
}
...
int MidiFile::addTrack(int count) {
   ...
   for (int i=0; i<count; i++) {
      ...
      events[length + i]->setSize(5000);
      events[length + i]->setSize(0);
      events[length + i]->allowGrowth(true);
   }
...
int MidiFile::read(const char* aFile) { 
   ...
   for (int z=0; z<tracks; z++) {
      ...
      events[z]->setAllocSize(5000);
      events[z]->setSize(0);
      events[z]->allowGrowth(true);
   }
...
void MidiFile::extractMidiData(FileIO& inputfile, StaticArray<uchar>& array, 
...
int _cdecl eventcompare(const void* a, const void* b) {
...
int _cdecl MidiFile::ticksearch(const void* A, const void* B) {
