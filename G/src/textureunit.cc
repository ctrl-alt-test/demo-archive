...
namespace Texture
{
  ...
  */
  ...
  void Unit::unUse()
  {
    ...
    // Apparemment c'est un truc qui n'a pas changé depuis dix ans
  ...
  */
  ...
  void Unit::setDynamicTexture(unsigned int width, unsigned int height,
			       GLenum format, GLenum internalFormat,
			       GLenum dataType
			       DBGARG(char * description))
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, _width, _height, 0, format,
		 dataType, _data);
    OGL_ERROR_CHECK("Création d'une texture vide");
    _activateFiltering();
    _activateTiling();
  }

  void Unit::updateDynamicTexture(int xoffset, int yoffset,
				  int width, int height,
				  GLenum dataType, const GLvoid * data) const
  {
    use();
    glTexSubImage2D(GL_TEXTURE_2D, 0, xoffset, yoffset, width, height, _format, dataType, data);
    OGL_ERROR_CHECK("glTexSubImage2D");
  }
...
//     setData(width, height, compressed ? GL_COMPRESSED_RGBA : GL_RGBA, data);
    setData(width, height, compressed ? GL_COMPRESSED_RGBA_S3TC_DXT5_EXT : GL_RGBA, data);
  }
  ...
  void Unit::load(bool freeData)
  {
	...
	GL_COMPRESSED_RGB_S3TC_DXT1_EXT == _format ||
	GL_COMPRESSED_RGBA_S3TC_DXT1_EXT == _format ||
	GL_COMPRESSED_RGBA_S3TC_DXT3_EXT == _format ||
	GL_COMPRESSED_RGBA_S3TC_DXT5_EXT == _format)
    {
      int width = 0;
      int height = 0;
      int depth = 0;
	...
	DBG("Texture non compressée !");
      ...
      DBG("Texture loaded: %dx%d, depth %d, border %d, red %d, green %d, blue %d, alpha %d, luminance %d, intensity %d, depth %d, %s, size %d",
