...
//_cinit(TRUE);
    ...
    const char* openglInitError = msys_init((intptr)s_wininfo.hWnd);
    if (openglInitError != NULL)
    {
      char errorMessage[256];
      msys_strcpy(errorMessage + msys_strlen(errorMessage), msg_errors[1]);
      msys_strcpy(errorMessage + msys_strlen(errorMessage), openglInitError);

      window_end();
      MessageBox(0, errorMessage, 0, MB_OK|MB_ICONEXCLAMATION);
      ExitProcess(0);
    }
    ...
    const int initResult = Intro::init(XRES, YRES, dlg.ratio(), dlg.credits(), font, sticker);
    if (initResult != 0)
    {
      window_end();
