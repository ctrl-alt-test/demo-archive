...
#include "../data/light/city.txt"
#include "../data/light/ramp.txt"
#include "../data/light/starfield.txt"
#include "../data/light/forest.txt"
  };
...
#include "../data/light/forest.txt"
                ...
                GLfloat * position)
  {
    ...
    */
    ...
    if (storyDate >= factoryStartDate && storyDate < factoryEndDate)
    {
      Factory::changeLight(storyDate,
			   ambient,
			   diffuse,
			   specular,
			   position);
    }
    else if (storyDate >= rampStartDate && storyDate < rampEndDate)
    {
      Ramp::changeLight(storyDate,
			ambient,
			diffuse,
			specular,
			position);
    }
    else if (storyDate >= forestStartDate && storyDate < forestEndDate)
    {
      Forest::changeLight(storyDate,
			  ambient,
			  diffuse,
			  specular,
			  position);
    }
    else if (storyDate >= elevationStartDate && storyDate < elevationEndDate)
    {
      Rope::changeLight(storyDate,
			ambient,
			diffuse,
			specular,
			position);
